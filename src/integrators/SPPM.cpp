...
bool SPPM::render(yafaray::imageFilm_t *image)
{
	...
	progressiveData.reserve(imageFilm->w*imageFile->h); //not possible, used for SPPM
	...
	for(int i=a.Y; i<end_y; ++i)
	{
		for(int j=a.X; j<end_x; ++j)
		{
			...
			for(int sample=0; sample<n_samples; ++sample) //set n_samples = 1.
			{
				...
				c_ray = camera->shootRay(j+dx, i+dy, lens_u, lens_v, wt); // wt是权重 需要查看它是如何计算的
				if(wt==0.0)
				{
				...
				int index = i*imageFilm->w + j; //not possible, w is a protected member.
				record &temp = progressiveData[index];
				...
				float g = min((temp.photoncount + _alpha * curPhotons) / (temp.photoncount + curPhotons), 1.0);
				temp.radius = QueryRadius * sqrt(g);
				temp.photoncount += curPhotons * _alpha;
				temp.accflux = (temp.accflux + col) * g;				
...
bool SPPM::prepass(int samples, int offset, bool adaptive)
{
	...
	while(!done)
	{
		...
		while( scene->intersect(ray, sp) ) //scatter photons.
		{
			...
			if(bsdfs & (BSDF_DIFFUSE))
			{
				...
				if(!causticPhoton)
				{
					photon_t np(wi, sp.P, pcol);// pcol作用
	...
	if(diffuseMap.nPhotons() > 0) //建立KD树
	{
...
colorA_t SPPM::integrate(renderState_t &state, diffRay_t &ray/*, sampler_t &sam*/) const
{
	...
	if(scene->intersect(ray, sp))
	{
        ...
        // remove FG here
		foundPhoton_t *gathered = (foundPhoton_t *)alloca(nSearch * sizeof(foundPhoton_t));//分配查找空间到的空间
		...
		if(nGathered > 0)
		{
			...
			float scale = 1.f / ( float(diffuseMap.nPaths()) * radius * M_PI); // scale for energy normolize, need to be changed!     see pbrt
			...
			if( bsdfs & BSDF_GLOSSY )
			{
				...
				for(int ns=0; ns<gsam; ++ns)
				{
					...
					if(s.pdf > 1.0e-6f && (s.sampledFlags & BSDF_GLOSSY))
					{
						mcol *= std::fabs(wi*sp.N)/s.pdf;
						refRay = diffRay_t(sp.P, wi, MIN_RAYDIST);
						gcol += (color_t)integrate(state, refRay) * mcol; //这句就递归下去了
					}
					if((bsdfs&BSDF_VOLUMETRIC) && (vol=material->getVolumeHandler(sp.N * refRay.dir < 0)))
					{
